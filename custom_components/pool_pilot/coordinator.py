...
class PoolPilotCoordinator(DataUpdateCoordinator[PoolPilotData]):
    ...
    def _scenario_steps(self, alert_id: str, product_name: str | None = None, product_qty: float | None = None, product_unit: str | None = None) -> list[str]:
        ...
        if alert_id == "water_cold":
            return [
                "Activez la pompe à chaleur si vous souhaitez remonter la température.",
                "Vérifiez la consigne de chauffage.",
                "Laissez la filtration fonctionner normalement.",
            ]
        if alert_id == "water_hot":
            return [
        ...
        if alert_id == "flipr_battery_low":
            return [
                ...
                "Vérifiez que le capteur remonte de nouveau ses mesures.",
            ]
        if alert_id == "stock_low":
            return [
    ...
    def _build_pool_alerts(self, water_temp: float | None, ph: float | None, orp: float | None, fc: float | None, weather_factor: float) -> list[dict[str, Any]]:
        ...
        if water_temp is not None and water_temp < min_temp:
            alerts.append({
                ...
                "badge": "snowflake",
                "color": "white",
                "steps": self._scenario_steps("water_cold"),
                "action_type": "heatpump",
            })
        ...
        if water_temp is not None and water_temp > max_temp:
            alerts.append({
                ...
                "badge": "thermometer",
                "color": "white",
                "steps": self._scenario_steps("water_hot"),
                "action_type": "heatpump_cooling",
            })
