...
SENSORS = (
    ...
    PoolPilotSensorDescription(key="auto_filter_remaining", translation_key="auto_filter_remaining", native_unit_of_measurement="h", state_class=SensorStateClass.MEASUREMENT, icon="mdi:timer-play-outline", value_fn=lambda d: d.auto_filter_remaining_hours, attrs_fn=lambda d: {"active": d.auto_filter_active, "end": d.auto_filter_end.isoformat() if d.auto_filter_end else None}),
    PoolPilotSensorDescription(key="auto_filter_status", translation_key="auto_filter_status", icon="mdi:lightning-bolt-circle", value_fn=lambda d: "active" if d.auto_filter_active else "idle", attrs_fn=lambda d: {"remaining_hours": d.auto_filter_remaining_hours, "end": d.auto_filter_end.isoformat() if d.auto_filter_end else None}),
    PoolPilotSensorDescription(key="auto_schedule_status", translation_key="auto_schedule_status", icon="mdi:calendar-clock", value_fn=lambda d: d.auto_schedule_status, attrs_fn=lambda d: {"enabled": d.auto_schedule_enabled, "windows": d.auto_schedule_windows, "next_start": d.auto_schedule_next_start.isoformat() if d.auto_schedule_next_start else None}),
    PoolPilotSensorDescription(key="auto_schedule_next_start", translation_key="auto_schedule_next_start", icon="mdi:clock-start", value_fn=lambda d: d.auto_schedule_next_start.isoformat() if d.auto_schedule_next_start else None, attrs_fn=lambda d: {"windows": d.auto_schedule_windows}),
    ...
    PoolPilotSensorDescription(key="strip_test", translation_key="strip_test", icon="mdi:test-tube", value_fn=lambda d: d.strip_test.get("updated_at", "Jamais"), attrs_fn=lambda d: dict(d.strip_test)),
